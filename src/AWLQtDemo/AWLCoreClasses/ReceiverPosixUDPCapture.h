#ifndef AWL_RECEIVER_POSIXUDP_CAPTURE_H
#define AWL_RECEIVER_POSIXUDP_CAPTURE_H
...
/** \brief TheReceiverCANCapture class is a specialized implementation of the ReceiverCapture
   *        used to acquire data from AWL LIDAR modules	through the PosixUDP CAN bus interface.
  * \author Jean-Yves Deschênes
  */
class ReceiverPosixUDPCapture: public ReceiverCANCapture
{
// Public types
public:
	typedef boost::shared_ptr<ReceiverPosixUDPCapture> Ptr;
    typedef boost::shared_ptr<ReceiverPosixUDPCapture> ConstPtr;

// public Methods
public:
	/** \brief ReceiverPosixUDPCapture constructor.
	...
	/** \brief ReceiverPosixUDPCapture constructor from a configuration file information.
 	    * \param[in] inReceiverID  unique receiverID
	    * \param[in] propTree propertyTree that contains teh confoguration file information.
      */

	ReceiverPosixUDPCapture(int receiverID,  boost::property_tree::ptree  &propTree);


	/** \brief ReceiverPosixUDPCapture Destructor.  Insures that all threads are stopped before destruction.
      */
	virtual ~ReceiverPosixUDPCapture();
...
protected:
		...
		std::string serverAddress;
	    	...
	    	/** \brief Server UDP port (ex: 23456) */
		int serverUDPPort;

	    	/** \brief Posix UDP data */
		void *remoteAddress;
...
#endif //AWL_RECEIVER_POSIXUDP_CAPTURE_H
