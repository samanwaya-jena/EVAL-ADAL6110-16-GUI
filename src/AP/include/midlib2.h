...
typedef enum  
{
     ...
     MI_CRC_ERROR               = 0x11,         //Command/response CRC error
...
typedef enum { MI_UNKNOWN_IMAGE_TYPE, 
               MI_BAYER_8,
               MI_BAYER_10,
               MI_BAYER_8_ZOOM_, // (deprecated, equivalent to MI_BAYER_8)
               MI_BAYER_10_ZOOM_, // (deprecated, equivalent to MI_BAYER_10)
               MI_YCBCR,
               MI_RGB565,
               MI_RGB555,
               MI_RGB444X,
               ...
               MI_BAYER_14,
               MI_BAYER_12_HDR_,  // Compressed HiDy on A-1000ERS (deprecated, equivalent to MI_BAYER_12)
               MI_BAYER_14_HDR_,  // Compressed HiDy on A-1000ERS (deprecated, equivalent to MI_BAYER_14)
               MI_BAYER_20,
               MI_RGB332,
               ...
               MI_BAYER_S24,  // Signed 24-bit, 0-16777215 nominal value range,
                              // intermediate image format used in software colorpipe
               MI_H264,
               MI_BAYER_18_,  //  Was never supported. Use MI_BAYER_20
               MI_BAYER_24,
...
typedef enum { MI_UNKNOWN_SENSOR,
               ...
               MI_RESERVED_SENSOR105,
               MI_RESERVED_SENSOR106,
               MI_RESERVED_SENSOR107,  // Reuse - was MI_AP0250 but that has been removed
               MI_RESERVED_SENSOR108,
               MI_RESERVED_SENSOR109,
               MI_RESERVED_SENSOR110,
               MI_RESERVED_SENSOR111,
               MI_RESERVED_SENSOR112,
               MI_RESERVED_SENSOR113,
               MI_RESERVED_SENSOR114,
               MI_RESERVED_SENSOR115,
               MI_RESERVED_SENSOR116,
               MI_RESERVED_SENSOR117,
               MI_RESERVED_SENSOR118,
               MI_RESERVED_SENSOR119,
               MI_RESERVED_SENSOR120,
               MI_RESERVED_SENSOR121,
               MI_RESERVED_SENSOR122,
               MI_RESERVED_SENSOR123,
               MI_RESERVED_SENSOR124,
               MI_RESERVED_SENSOR125,
               MI_RESERVED_SENSOR126,
               MI_RESERVED_SENSOR127,
               MI_RESERVED_SENSOR128,
               MI_RESERVED_SENSOR129,
               MI_RESERVED_SENSOR130,
               //  Only add symbols to the end
} mi_sensor_types;
...
#define MI_AR0135    MI_RESERVED_SENSOR100 /* For AT and CS variants */
#define MI_AR0140    MI_RESERVED_SENSOR66  /* AR0140AT and AR0140CS */
#define MI_AR0141    MI_RESERVED_SENSOR91  /* AR0141CS */
#define MI_AR0230    MI_RESERVED_SENSOR90  /* AR0230CS, and AR023Z variant */
#define MI_AR0237    MI_RESERVED_SENSOR103 /* AR0238 variant. CS only, but doesn't use CS extension in name */
...
#define MI_AR1335    MI_RESERVED_SENSOR88  /* AR1335, AR1335HS */
#define MI_AR1337    MI_RESERVED_SENSOR102
...
#define MI_ASX340    MI_RESERVED_SENSOR46  /* ASX340AT, ASX340CS, ASX342AT - 361_SOC */
...
#define MI_MT9M024   MI_1000ERS            /* Also MT9M034*/
...
typedef enum { MI_ERROR_CHECK_MODE,
               ...
               MI_TRIGGER0_HIGH_WIDTH,  // trigger high width in clocks
               MI_TRIGGER0_LOW_WIDTH,   // trigger low width in clocks
               ...
               MI_RX_WIDTH,  //  native image row width pixels (for HiSPi when using interleaved HDR)
               MI_SPI_REG_ACCESS,  //  Use SPI commands instead of I2C commands
               MI_SHIP_TO_SPI,  //  Convert I2C to SPI to access SPI sensor with I2C commands

               MI_TRIGGER1_HIGH_WIDTH,  // trigger1 high width in clocks
               MI_TRIGGER1_LOW_WIDTH,   // trigger1 low width in clocks
               MI_TRIGGER2_HIGH_WIDTH,  // trigger2 high width in clocks
               MI_TRIGGER2_LOW_WIDTH,   // trigger2 low width in clocks

               MI_SENDCOMMAND_TIMEOUT,   //  timeout value for sendCommand()
               MI_LOCK_SLAVE_IDS,   // don't update SERDES I2C forwarding/aliasing
               MI_MEM_BUFFERING,    // set number of frames to buffer on FrameBuffer

               MI_TRIGGER_COUNT, // for single shot, how many pulses
               MI_TRIGGER_INVERT, // invert trigger pulses (for each trigger bitwise)
               //  Only add symbols to the end
} mi_modes;
...
*****************************************************************************/
typedef struct mi_addr_space_val_t
{
    mi_string           ID;
    mi_string           name;
    mi_u32              val;
...
*****************************************************************************/
typedef struct mi_addr_space_t
{
...
/****************************************************************************
  ...
  This structure is used to hold the "long description" and/or "confidential
  notes" of a register or bitfield.

  regName           register name
  bitfieldName      bitfield name if this is a bitfield
  longDesc          long description, UTF-8
*****************************************************************************/
typedef struct mi_long_text_t
{
    char *          regName;
    char *          bitfieldName;
    char *          longText;
...
/****************************************************************************
  ...
  conf_notes           confidential notes, from ldat file
  productVariant       string for optional variant name

*****************************************************************************/
typedef struct mi_sensor_t
{
    ...
    struct mi_sensor_t *    far1_sensor;
    struct mi_sensor_t *    far2_sensor;
    mi_s32                  num_long_desc;
    mi_long_text_t *        long_desc;
    mi_string               productVariant;
    mi_s32                  num_conf_notes;
    mi_long_text_t *        conf_notes;
} mi_sensor_t;
...
*****************************************************************************/
typedef struct mi_frame_data_t
{
...
typedef struct mi_camera_t
{
    ...
    mi_s32      (* startTransport)(struct mi_camera_t *pCamera);
    mi_s32      (* stopTransport)(struct mi_camera_t *pCamera);
    mi_s32      (* readSensorRegisters)(struct mi_camera_t *pCamera, mi_u32 addrSpace, mi_u32 startAddr, mi_u32 numRegs, mi_u32 vals[]);
    mi_s32      (* writeSensorRegisters)(struct mi_camera_t *pCamera, mi_u32 addrSpace, mi_u32 startAddr, mi_u32 numRegs, mi_u32 vals[]);
    void *          _reserved0;// was mi_s32 (* readSensorRegList)(struct mi_camera_t *pCamera, mi_u32 numRegs, mi_u32 addrSpaces[], mi_u32 regAddrs[], mi_u32 vals[]);
    mi_s32      (* readRegister)(struct mi_camera_t *pCamera, mi_u32 shipAddr, mi_u32 regAddr, mi_u32 *val);
    mi_s32      (* writeRegister)(struct mi_camera_t *pCamera, mi_u32 shipAddr, mi_u32 regAddr, mi_u32 val);
    mi_s32      (* readRegisters)(struct mi_camera_t *pCamera, mi_u32 shipAddr, mi_u32 startAddr, mi_u32 numRegs, mi_u32 vals[]);
    mi_s32      (* writeRegisters)(struct mi_camera_t *pCamera, mi_u32 shipAddr, mi_u32 startAddr, mi_u32 numRegs, mi_u32 vals[]);
    mi_s32      (* grabFrame)(struct mi_camera_t *pCamera, mi_u8 *pInBuffer, mi_u32 bufferSize);
    mi_s32      (* getFrameData)(struct mi_camera_t *pCamera, mi_frame_data_t *frameData);
    mi_s32      (* updateFrameSize)(struct mi_camera_t *pCamera, mi_u32 width, mi_u32 height, mi_s32 nBitsPerClock, mi_s32 nClocksPerPixel);
    mi_s32      (* updateBufferSize)(struct mi_camera_t *pCamera, mi_u32 rawBufferSize);
    mi_s32      (* setMode)(struct mi_camera_t *pCamera, mi_modes mode, mi_u32 val);
    mi_s32      (* getMode)(struct mi_camera_t *pCamera, mi_modes mode, mi_u32* val);
    mi_s32      (* initTransport)(struct mi_camera_t *pCamera, mi_s32 bitsPerClock, mi_s32 clocksPerPixel, mi_s32 polarity, mi_s32 pixelOffset, mi_s32 noFWCalls);
} mi_camera_t;
...
typedef struct mi_notify_updateframesize_t
{
    mi_u32          width;
    mi_u32          height;
    mi_s32          bitsPerClock;
...
typedef mi_s32 (*MI_LOGCALLBACK)(mi_u32 logType, mi_s32 errorCode, const char *logMsg, const char *szSource, const char *szFunc, mi_u32 nLine);
...
#define MI_CCR_IT_BAYER_ODDH    2 // bayer / rccb ordering
#define MI_CCR_IT_BAYER_ODDV    4 // bayer / rccb ordering
//  the following for version 0x0103+
#define MI_CCR_IT_RCCB          0x02
#define MI_CCR_IT_YUV           0x03
#define MI_CCR_IT_RCB           0x04
#define MI_CCR_IT_CEF           0x05
#define MI_CCR_IT_STREAM        0x06
#define MI_CCR_IT_GRAY          0x07
#define MI_CCR_IT_MASK          0xff
#define MI_CCR_IST_LIN          0x000 // data is in linear space
#define MI_CCR_IST_LS4_9        0x100 // data is in log s4.9 space
#define MI_CCR_IST_LS3_9        0x200 // data is in log s3.9 space
#define MI_CCR_IST_MASK         0xf00 // subtype mask
#define MI_CCR_IST_ODDH         0x2000 // bayer / rccb ordering
#define MI_CCR_IST_ODDV         0x4000 // bayer / rccb ordering
#define MI_CCR_ISS_CSSH         0x10000  // chroma subsampling in dimension x
#define MI_CCR_ISS_CSSV         0x20000  // chroma subsampling in dimension y
#define MI_CCR_ISS_CALH         0x40000  // chroma alignment in dimension x
#define MI_CCR_ISS_CALV         0x80000  // chroma alignment in dimension y
#define MI_CCR_IF_IMG_FLIPH     0x100000
#define MI_CCR_IF_IMG_FLIPV     0x200000
typedef struct mi_ccr_header_t
{
    mi_u32      ccrID;          //  MI_CCR_ID = 0x0a524343 = 'CCR\n'
    mi_u32      ccrVersion;     //  MI_CCR_VERSION = 0x102 or 0x103
    mi_u32      width;
    mi_u32      height;
    mi_u32      imageType;      //  MI_CCR_IT_BAYER for Bayer
    mi_u32      bitsPerPixel;
    double      gamma;          //  1.0
...
typedef struct mi_dxr_header_t
{
...
/****************************************************************************
  ...
  mi_IsMcuSpace         - returns true if the address space is in the FW variables
...
MIDLIB_API mi_s32           mi_IsMARs(mi_camera_t* pCamera);
MIDLIB_API mi_s32           mi_IsISP(mi_camera_t* pCamera);
MIDLIB_API mi_s32           mi_IsArrayImager(mi_camera_t* pCamera);
MIDLIB_API mi_s32           mi_IsMcuSpace(mi_addr_space_val_t *pSpace);
MIDLIB_API mi_s32           mi_IsFarSpace(mi_addr_space_val_t *pSpace);
MIDLIB_API mi_u32           mi_varMaskToIs8(mi_u32 nBitmask);
MIDLIB_API mi_u32           mi_dataSize(mi_camera_t *pCamera, mi_reg_data_t *pReg);
...
MIDLIB_API const char *     mi_GetSerdesPartNumber(mi_camera_t *pCamera);
...
MIDLIB_API mi_s32           mi_GetConfidentialNotes(mi_camera_t *pCamera, const char *pszRegisterName, const char *pszBitfieldName, char *szConfNotes, mi_s32 bufferLen);
...
MIDLIB_API void             mi_OpenErrorLogCallback(mi_u32 errorlog_level, MI_LOGCALLBACK logCallback);
...
#define mi_error(errorcode,logMsg)\
    mi_Errorlog(errorcode,-1,logMsg,__FILE__, __FUNCTION__,__LINE__)
#define mi_error_sev(errorcode,errorlevel,logMsg)\
    mi_Errorlog(errorcode,errorlevel,logMsg, __FILE__, __FUNCTION__,__LINE__)
