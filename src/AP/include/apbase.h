...
enum  
{
     ...
     AP_TOO_MUCH_DATA_ERROR     = MI_TOO_MUCH_DATA_ERROR    ,
     AP_CAMERA_NOT_SUPPORTED    = MI_CAMERA_NOT_SUPPORTED   ,
     AP_CRC_ERROR               = MI_CRC_ERROR              ,
...
enum
{
    ...
    AP_FLAG_OUTSIDE_REG_MASK      = 0x1000,    //  Writing outside the register mask (setting undefined bits)
    AP_FLAG_H264_RESTART          = 0x2000,    //  Restart H.264 decoding (or similar)
    AP_FLAG_USERCANCEL            = 0x8000,    //  User cancelled register write
};
...
APBASE_API void APBASE_DECL
ap_SetIoLogFlags(ap_u32 nLogFlags);
...
APBASE_API ap_s32 APBASE_DECL
ap_RunPython(AP_HANDLE apbase, const char *szStatements);

APBASE_API ap_s32 APBASE_DECL  //  returns AP_FLAG's
ap_CheckSensorState(AP_HANDLE apbase, unsigned int nCheckFlags);

APBASE_API ap_s32 APBASE_DECL
ap_GetRegisterSpy(AP_HANDLE apbase);

APBASE_API ap_s32 APBASE_DECL
ap_SetRegisterSpy(AP_HANDLE apbase, ap_s32 bEnable);
...
APBASE_API const char *  APBASE_DECL
ap_GetMouseSelection(AP_HANDLE apbase, ap_s32 *pStartX, ap_s32 *pStartY, ap_s32 *pEndX, ap_s32 *pEndY);

APBASE_API ap_s32 APBASE_DECL
ap_SetMouseSelection(AP_HANDLE apbase, const char *szSelectType,
                     ap_s32 nStartX, ap_s32 nStartY, ap_s32 nEndX, ap_s32 nEndY);
...
APBASE_API ap_s32 APBASE_DECL  //  returns midlib error code
ap_SendCommand(AP_HANDLE    apbase,
               ap_u32       nCommand,
               ap_u32       nParamSize, unsigned char *pParamBuffer,
               ap_u32 *     pStatusCode,
               ap_u32       nResultSize, unsigned char *pResultBuffer,
               ap_u32 *     pBytesReturned);

APBASE_API ap_u32 APBASE_DECL
ap_SystemDump(AP_HANDLE     apbase,
              char *        pBuffer,
              ap_u32        nBufferSize,
              const char *  szAppFileName);
...
APBASE_API ap_s32 APBASE_DECL
ap_GrabFrameAndSave(AP_HANDLE apbase);
...
APBASE_API ap_s32 APBASE_DECL
ap_ExistsRegister(AP_HANDLE   apbase,
                  const char *szDevice,
                  const char *szRegister);

APBASE_API ap_s32 APBASE_DECL
ap_ExistsBitfield(AP_HANDLE   apbase,
                  const char *szDevice,
                  const char *szRegister,
...
typedef void            (APBASE_DECL * AP_SetSensorRegister_PROC )(void *pContext, mi_addr_type nAddrType, ap_u32 nAddrSpace, ap_u32 nAddr, ap_u32 nDataBits, ap_u32 nValue, int bSpaceMask);
typedef void            (APBASE_DECL * AP_SetSensorRegBurst_PROC )(void *pContext, mi_addr_type nAddrType, ap_u32 nAddrSpace, ap_u32 nAddr, ap_u32 nSpan, ap_u32 *pnValues, int nCount);
typedef ap_s32          (APBASE_DECL * AP_BeforeSetSensorReg_PROC)(void *pContext, const char *szRegisterName, ap_u32 nValue, ap_s32 nSideEffects);
typedef void            (APBASE_DECL * AP_AfterSetSensorReg_PROC )(void *pContext, const char *szRegisterName, ap_u32 nValue, ap_s32 nSideEffects);
...
typedef ap_s32          (APBASE_DECL * AP_MultipleChoice_PROC    )(void *pContext, const char *szMessage, const char *szChoices);
typedef void            (APBASE_DECL * AP_LogDelay_PROC          )(void *pContext, int nDelay);
typedef void            (APBASE_DECL * AP_LogSensorRegPoll_PROC  )(void *pContext, const char *szRegisterName, ap_u32 nMask, const char *szOperator, ap_u32 nNum, ap_u32 nDelay, ap_u32 nTimeout, mi_u32 *pnValues, int nCount);
typedef void            (APBASE_DECL * AP_LogComment_PROC        )(void *pContext, const char *szComment);
typedef ap_s32          (APBASE_DECL * AP_LogDebugTrace_PROC     )(void *pContext, const char *szTrace);
typedef ap_s32          (APBASE_DECL * AP_ErrorMessage_PROC      )(void *pContext, const char *szMessage, unsigned int mbType);
typedef ap_s32          (APBASE_DECL * AP_ProgressReport_PROC    )(void *pContext, const char *szMessage, int progress);
...
typedef int             (APBASE_DECL * AP_RefreshSensorFile_PROC )(void *pContext, const char *szString);
typedef int             (APBASE_DECL * AP_SetFarSensorFile_PROC  )(void *pContext, int nIndex, unsigned int nFarBase, const char *szString);

APBASE_API void APBASE_DECL ap_SetCallback_SetSensorRegister (AP_HANDLE apbase, AP_SetSensorRegister_PROC  pProc, void *pContext);
APBASE_API void APBASE_DECL ap_SetCallback_SetSensorRegBurst (AP_HANDLE apbase, AP_SetSensorRegBurst_PROC  pProc, void *pContext);
APBASE_API void APBASE_DECL ap_SetCallback_BeforeSetSensorReg(AP_HANDLE apbase, AP_BeforeSetSensorReg_PROC pProc, void *pContext);
APBASE_API void APBASE_DECL ap_SetCallback_AfterSetSensorReg (AP_HANDLE apbase, AP_AfterSetSensorReg_PROC  pProc, void *pContext);
...
APBASE_API void APBASE_DECL ap_SetCallback_LogDelay          (AP_HANDLE apbase, AP_LogDelay_PROC           pProc, void *pContext);
APBASE_API void APBASE_DECL ap_SetCallback_LogSensorRegPoll  (AP_HANDLE apbase, AP_LogSensorRegPoll_PROC   pProc, void *pContext);
...
APBASE_API void APBASE_DECL ap_SetCallback_RefreshSensorFile (AP_HANDLE apbase, AP_RefreshSensorFile_PROC  pProc, void *pContext);
APBASE_API void APBASE_DECL ap_SetCallback_SetFarSensorFile  (AP_HANDLE apbase, AP_SetFarSensorFile_PROC   pProc, void *pContext);

//  AP_ErrorMessage_PROC mbType parameter, based on Windows MessageBox
enum  //  mbType & 0xF
{
    AP_MSGTYPE_OK                   = 0,
...
//  AP_ErrorMessage_PROC return values, based on Windows MessageBox
enum
{
    AP_MSG_OK           =  1,
    AP_MSG_CANCEL       =  2,
