#ifndef AWL_RECEIVER_SOCKETCAN_CAPTURE_H
#define AWL_RECEIVER_SOCKETCAN_CAPTURE_H
...
/** \brief TheReceiverCANCapture class is a specialized implementation of the ReceiverCapture
   *        used to acquire data from AWL LIDAR modules	through the SocketCAN CAN bus interface.
  * \author Jean-Yves Deschênes
  */
class ReceiverSocketCANCapture: public ReceiverCANCapture
{
// Public types
public:
	typedef boost::shared_ptr<ReceiverSocketCANCapture> Ptr;
    typedef boost::shared_ptr<ReceiverSocketCANCapture> ConstPtr;

// public Methods
public:
	/** \brief ReceiverSocketCANCapture constructor.
	...
	ReceiverSocketCANCapture(int receiverID, int inReceiverChannelQty, int inReceiverColumns, int inReceiverRows, float inLineWrapAround, 
	...
	/** \brief ReceiverSocketCANCapture constructor from a configuration file information.
 	    * \param[in] inReceiverID  unique receiverID
	    * \param[in] propTree propertyTree that contains teh confoguration file information.
      */

	ReceiverSocketCANCapture(int receiverID,  boost::property_tree::ptree  &propTree);


	/** \brief ReceiverSocketCANCapture Destructor.  Insures that all threads are stopped before destruction.
      */
	virtual ~ReceiverSocketCANCapture();
...
protected:
	...
	/** \brief Converts the ReceiverCANCapture canRate into a SocketCAN-specific rate string, stored in the canBitRateCode member.
      */
...
protected:
		...
		int closeCANReentryCount;
	        ...
	        /** \brief CAN BitRate. Default is 1000000*/
		long canBitRate;
};

} // namespace AWL

#endif //AWL_RECEIVER_SOCKETCAN_CAPTURE_H
